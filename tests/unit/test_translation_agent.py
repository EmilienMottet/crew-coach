...
class TestTranslationAgent:
    ...
    def test_title_translated_to_french(self, crew_instance, sample_activity_outside_work):
    ...
    @pytest.mark.timeout(120)
    def test_description_translated_to_french(self, crew_instance, sample_activity_outside_work):
    ...
    @pytest.mark.timeout(120)
    def test_character_limits_respected(self, crew_instance, sample_activity_outside_work):
    ...
    @pytest.mark.timeout(120)
    def test_emoji_preserved_in_translation(self, crew_instance, sample_activity_outside_work):
    ...
    @pytest.mark.timeout(120)
    def test_metrics_preserved_format(self, crew_instance, sample_activity_outside_work):
        ...
        has_metric = any(re.search(pattern, desc, re.IGNORECASE) for pattern in metric_patterns)
    ...
    def test_run_activity_french_terminology(self, crew_instance):
        ...
        running_terms = ["course", "sortie", "entraînement", "allure", "tempo", "foulée"]
        has_running_term = any(term in combined_lower for term in running_terms)

        assert has_running_term, \
