...
class ValidatedIngredientInput(BaseModel):
    ...
    name: str = Field(..., description="Ingredient name with quantity (e.g., '120g chicken breast')")
    passio_food_id: Optional[str] = Field(None, description="Passio Food ID from hexis_search_passio_foods")
    passio_food_name: Optional[str] = Field(None, description="Exact name from Passio database")
...
class HexisLogMealTool(BaseTool):
    ...
    def __init__(self, verify_tool, update_tool, get_plan_tool, search_tool, create_tool=None):
        super().__init__()
    ...
    def _extract_ingredients_from_description(self, description: str) -> List[str]:
        ...
        common_foods = [
            'chicken', 'beef', 'salmon', 'tuna', 'eggs', 'egg',
            'rice', 'pasta', 'bread', 'oatmeal', 'oats',
            'yogurt', 'greek yogurt', 'milk', 'cheese',
            'broccoli', 'spinach', 'vegetables', 'salad',
            'banana', 'apple', 'berries', 'avocado',
            'almonds', 'nuts', 'peanut butter',
            'olive oil', 'butter'
        ]

        description_lower = description.lower()
        for food in common_foods:
            if food in description_lower and food not in [i.lower() for i in ingredients]:
        ...
        try:
            ...
            if validated_ingredients and len(validated_ingredients) > 0:
                ...
                for ing in validated_ingredients:
                    ing_data = ing if isinstance(ing, dict) else ing.model_dump() if hasattr(ing, 'model_dump') else {}
                    ...
                    if ing_data.get("passio_food_id"):
                        ...
                        data_origin = "PASSIO"
                        custom_food_name = ing_data.get("passio_food_name", meal_name_input)
                        if ref_code:
                            print(f"✅ Using pre-validated foodId: {food_id} with refCode ({custom_food_name})")
                        else:
                            print(f"⚠️  Using pre-validated foodId: {food_id} WITHOUT refCode ({custom_food_name})")
                        break

                if not food_id:
                    print(f"⚠️  validated_ingredients provided but no passio_food_id found, falling back to search")
            ...
            if food_id:
                ...
                try:
                    ...
                    elif isinstance(search_result, dict) and "data" in search_result and isinstance(search_result["data"], list):
                    ...
                    if found_food:
                        ...
                        ref_code = found_food.get("refCode")  # CRITICAL: capture refCode from search
                        data_origin = found_food.get("dataOrigin", "PASSIO")
                        custom_food_name = found_food.get("name", meal_name_input)
                        if ref_code:
                            print(f"✅ Found food '{found_food.get('name')}' with refCode: {ref_code[:40]}...")
                        else:
                            print(f"⚠️  Found food '{found_food.get('name')}' but NO refCode (ID: {food_id})")
                    else:
                        # Strategy 3b: Fallback - extract ingredients from description and retry search
                        print(f"⚠️  No food found for meal name '{search_query}'. Trying ingredient extraction fallback...")
                        description = kwargs.get("description", "")
                        extracted_ingredients = self._extract_ingredients_from_description(description)

                        if extracted_ingredients:
                            print(f"🔍 Extracted {len(extracted_ingredients)} ingredient candidates: {extracted_ingredients[:3]}")

                            for ingredient_query in extracted_ingredients:
                                print(f"🔍 Fallback search for ingredient: '{ingredient_query}'")
                                try:
                                    fallback_result = self._search_tool.run(query=ingredient_query)
                                    ...
                                    elif isinstance(fallback_result, dict) and "data" in fallback_result:
                                        fallback_foods = fallback_result.get("data", [])
                                    elif isinstance(fallback_result, dict) and "foods" in fallback_result:
                                        fallback_foods = fallback_result.get("foods", [])
                                    ...
                                    if fallback_foods:
                                        ...
                                        data_origin = found_food.get("dataOrigin", "PASSIO")
                                        custom_food_name = found_food.get("name") or found_food.get("displayName") or ingredient_query
                                        print(f"✅ Fallback SUCCESS: Found '{custom_food_name}' for ingredient '{ingredient_query}'")
                                        break
                                except Exception as fallback_err:
                                    print(f"⚠️  Fallback search error for '{ingredient_query}': {fallback_err}")
                                    continue

                        if not found_food:
                            print(f"❌ No food found for '{search_query}' and fallback extraction failed")
            ...
            print(f"📦 food_object: foodId={food_id}, quantity={quantity_g}g, dataOrigin={data_origin or 'PASSIO'}")
...
def create_hexis_log_meal_tool(hexis_tools: List[BaseTool]) -> HexisLogMealTool:
    ...
    search_tool = next((t for t in hexis_tools if t.name == "hexis__hexis_search_passio_foods"), None)
    ...
    if not all([verify_tool, update_tool, get_plan_tool, search_tool]):
        ...
        if not search_tool: missing.append("hexis_search_passio_foods")
